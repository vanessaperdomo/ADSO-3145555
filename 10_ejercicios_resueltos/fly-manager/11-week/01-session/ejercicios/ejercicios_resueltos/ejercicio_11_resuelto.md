# Ejercicio 11 Resuelto - Automatización del check-in y generación del boarding pass
...
Se requiere automatizar el flujo de abordaje una vez el pasajero realiza check-in. La meta es que el registro operativo quede completo en dos niveles: la marca del `check_in` y la generación inmediata del `boarding_pass`.
...
### IDENTITY
**Entidades:** `person`  
**Propósito en este ejercicio:** identificar al pasajero que realizará el abordaje.
...
**Propósito en este ejercicio:** identificar al usuario que ejecuta el check-in.
...
5. El trigger se dispara automáticamente.
...
- la consulta usa `INNER JOIN` en más de 5 tablas,
- el trigger es `AFTER INSERT`,
- el procedimiento almacenado es reutilizable,
- la demostración prueba la ejecución completa del flujo.
